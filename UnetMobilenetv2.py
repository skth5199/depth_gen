...
#U-net with Mobilenetv2 encoder Architecture
...
class Decoder(nn.Module):
    def __init__(self, featurescnt=1280, dw = .6):
        ...
        self.upspl2 = upsampleBlock(skin=pms//2+160, opFeature=pms//2)
        self.upspl3 = upsampleBlock(skin=pms//2+64, opFeature=pms//4)
        self.upspl4 = upsampleBlock(skin=pms//4+32, opFeature=pms//8)
        self.upspl5 = upsampleBlock(skin=pms//8+24, opFeature=pms//8)
        self.upspl6 = upsampleBlock(skin=pms//8+16, opFeature=pms//16)
    ...
    def forward(self, pms):
        ...
        y0 = self.convol2(l6)
        y1 = self.upspl1(y0, l5)
        y2 = self.upspl2(y1, l4)
        y3 = self.upspl3(y2, l3)
        y4 = self.upspl4(y3, l2)
        y5 = self.upspl5(y4, l1)
        y6 = self.upspl6(y5, l0)
        return self.convol3(y6)

class Encoder(nn.Module):
    def __init__(self):
        super(Encoder, self).__init__()      
        self.ptmodel = models.mobilenet_v2(pretrained=True)
